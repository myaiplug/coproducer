...
def license_check() -> dict[str, Any]:
    try:
        ...
        if status.source:
            detail = f"{detail} (source: {status.source})"
        return check("License activated", status.is_valid, detail)
    except Exception as exc:
        return check("License activated", False, f"license check error: {exc}")


def diagnostic_checks(
    paths: ProjectPaths, settings: dict[str, Any], ffmpeg: FFmpeg
) -> list[dict[str, Any]]:
    ...
    if dependencies["ffmpeg"]:
        for profile in settings["previews"]["codec_profiles"]:
            ...
            checks.append(
                check(f"Encoder {encoder}", ffmpeg.encoder_available(encoder), "preview feature")
            )
    return checks
