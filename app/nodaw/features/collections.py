...
def album_consistency(
    ...
    dynamic_values = [
        row["dynamic_range_db"] for row in rows if row["dynamic_range_db"] is not None
    ]
    medians = {
    ...
    for row in rows:
        ...
        row["dynamic_range_delta"] = difference(
            row["dynamic_range_db"], medians["dynamic_range_db"]
        )
