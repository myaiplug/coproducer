...
def analyze_codecs(
    ...
    for profile in settings["previews"]["codec_profiles"]:
        ...
        if generate_previews and available:
            destination = output_dir / f"{safe_name(source.stem)}_{safe_name(profile['name'])}{profile['extension']}"
            arguments = [
                "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
                "-i", str(source), "-t", str(duration), "-vn", "-c:a", encoder,
                *[str(value) for value in profile.get("args", [])],
                str(destination),
            ]
            ffmpeg.run_checked(arguments)
            item.update(status="generated", path=str(destination), size_bytes=destination.stat().st_size)
        elif not available:
