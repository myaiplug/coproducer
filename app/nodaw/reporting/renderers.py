...
def waveform_svg(values: list[float]) -> str:
    ...
    bottom = [
        (index * step, middle + value * 76) for index, value in reversed(list(enumerate(values)))
    ]
...
def findings_section(findings: list[dict[str, Any]]) -> str:
    ...
    return (
        "<section class='panel'><h2>Engineering findings</h2><div class='findings'>"
        + "".join(cards)
        + "</div></section>"
    )
...
def table_section(title: str, rows: list[dict[str, Any]]) -> str:
    ...
    body = "".join(
        "<tr>"
        + "".join(f"<td>{escape(display(row.get(column)))}</td>" for column in columns)
        + "</tr>"
        for row in rows
    )
...
def repairs_section(repairs: list[dict[str, Any]]) -> str:
    ...
    return (
        "<section class='panel'><h2>Repair recommendations</h2>" + "".join(content) + "</section>"
    )
...
def render_html(report: dict[str, Any]) -> str:
    ...
    if ref_match.get("debug"):
        ...
        body.append(
            table_section("Reference Match Debug (deltas, severity, contributions)", debug_rows)
        )
        if db.get("explanation"):
            body.append(
                f"<section class='panel'><h2>Score Explanation</h2><p>{escape(db['explanation'])}</p></section>"
            )
    ...
    score_markup = (
        ...
        else "<div class='score neutral'><strong>-</strong><span>Informational report</span></div>"
    )
...
def render_text(report: dict[str, Any]) -> str:
    ...
    if findings:
        ...
        for finding in findings:
            lines.append(
                f"[{finding['severity'].upper()}] {finding['title']}: {finding['message']}"
            )
