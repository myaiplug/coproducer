...
DEFAULT_SETTINGS: dict[str, Any] = {
    "brand": APP_NAME,
    "supported_extensions": [".wav", ".mp3", ".flac", ".m4a", ".aac", ".ogg", ".opus", ".aiff", ".aif"],
    "analysis": {
        "target_lufs": -14.0,
    ...
    "previews": {
        "duration_seconds": 30,
        "codec_profiles": [
            {"name": "MP3 320 kbps", "extension": ".mp3", "codec": "libmp3lame", "args": ["-b:a", "320k"]},
            {"name": "AAC 256 kbps", "extension": ".m4a", "codec": "aac", "args": ["-b:a", "256k"]},
            {"name": "Opus 160 kbps", "extension": ".opus", "codec": "libopus", "args": ["-b:a", "160k"]}
        ]
    },
    "streaming_profiles": {
...
def load_settings(paths: ProjectPaths) -> dict[str, Any]:
    ...
    for name, limits in bands.items():
        if not isinstance(limits, list) or len(limits) != 2 or limits[0] <= 0 or limits[1] <= limits[0]:
