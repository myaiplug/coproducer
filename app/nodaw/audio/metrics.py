...
class MetricsAnalyzer:
    ...
    def _stereo_width(self, path: Path, channels: int) -> float | None:
        ...
        result = self.ffmpeg.run_checked([
            "ffmpeg", "-hide_banner", "-nostats", "-i", str(path),
            "-af", "pan=stereo|c0=0.5*c0+0.5*c1|c1=0.5*c0-0.5*c1,astats=metadata=0:reset=0",
            "-f", "null", "-",
        ])
    ...
    def _phase_correlation(self, path: Path, channels: int) -> float | None:
        ...
        values = [
            float(value)
            for value in re.findall(rf"lavfi\.aphasemeter\.phase=({NUMBER})", result.stdout + result.stderr)
        ]
        if not values:
            return None
    ...
    def _waveform(self, path: Path) -> list[float]:
        ...
        envelope = [max(abs(value) for value in samples[index:index + chunk]) for index in range(0, len(samples), chunk)]
