...
class FFmpeg:
    ...
    def run(self, arguments: Sequence[str], timeout: int | None = None) -> CommandResult:
        try:
            ...
            raise FFmpegError(f"Command timed out after {timeout or self.timeout_seconds} seconds.") from exc
    ...
    def run_bytes(self, arguments: Sequence[str], timeout: int | None = None) -> bytes:
        ...
        if process.returncode:
            ...
            raise FFmpegError(f"{arguments[0]} failed with exit code {process.returncode}: {detail}")
