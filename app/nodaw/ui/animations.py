...
def fade_in(widget: QWidget, duration: int = Duration.FADE) -> QPropertyAnimation:
    ...
    Keeps a strong reference on the widget so Qt does not GC the animation
    mid-run (which left panels stuck at opacity 0 — e.g. Reference Match results).
    Clears the opacity effect when finished so later hide/show works normally.
    """
    if widget is None:
        return None  # type: ignore[return-value]

    # Cancel any prior fade on this widget
    ...
    # Parent the animation to the widget so lifetime is tied to it
    ...
    def _finish():
        try:
            # Drop the effect so opacity cannot stick at 0 after GC / reparent
    ...
    # Strong refs so GC never kills the run
    widget._fade_anim = anim
    widget._fade_effect = eff
    anim.start()
    return anim
