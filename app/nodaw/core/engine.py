...
from .scoring import codec_recommendations, evaluate_track, rating, streaming_compatibility
...
class WorkflowRunner:
    ...
    def analyze(self, path: Path) -> TrackAnalysis:
        ...
        if key not in self._cache:
            ...
            try:
                ...
                self.logger.warning("Advanced analyzer failed (%s), falling back to legacy metrics", exc)
    ...
    def write_report(self, report: dict[str, Any], base_name: str, record: bool = True) -> dict[str, Path]:
    ...
    def single(self, input_path: Path | None = None) -> dict[str, Any]:
    ...
    def reference(self, input_path: Path | None = None, reference_path: Path | None = None) -> dict[str, Any]:
        source = self.source(input_path, self.paths.song_input)
        ...
        launcher = self.paths.exports / "repairs" / f"APPLY_{safe_name(source.stem)}_REFERENCE_MATCH.bat"
    ...
    def codecs(self, input_path: Path | None = None) -> dict[str, Any]:
        ...
        analysis = analyze_codecs(
            track, source, self.paths.exports / "previews" / "codecs",
            self.ffmpeg, self.settings, self.generate_previews,
        )
        score, report_rating, summary, findings = evaluate_track(track, self.settings)
    ...
    def streaming(self, input_path: Path | None = None) -> dict[str, Any]:
        ...
        analysis = analyze_streaming(
            track, source, self.paths.exports / "previews" / "streaming",
            self.ffmpeg, self.settings, self.generate_previews,
        )
        score, report_rating, summary, findings = evaluate_track(track, self.settings)
    ...
    def export(self) -> dict[str, Any]:
        ...
        report.update({
            ...
            "operations": [{"operation": "export report bundle", "status": "prepared", "path": str(destination)}],
        })
    ...
    def complete(
        ...
        def execute(name: str, callback: Any, required: bool = False) -> None:
            try:
                result = callback()
                operations.append({"operation": name, "status": "completed", "report_type": result["report_type"]})
        ...
        execute("streaming readiness and previews", lambda: self.streaming(input_path), required=True)
